...
DictRepr: TypeAlias = dict[str, Any] | str

# Load the JSON-LD context
_context_path = (
    Path(__file__).parent.parent.parent / "_out" / "transforms.context.jsonld"
)
...
def to_json(
    transform: Transform | Sequence[Transform], indent: int | None = None
) -> str:
    """
...
def to_jsonld(
    ...
    if include_context:
        ...
        if isinstance(data, list):
            ...
            jsonld_data = {"@context": context, "transforms": data}
        elif isinstance(data, dict):
            jsonld_data = {"@context": context, **data}
        else:
            jsonld_data = {"@context": context, "@value": data}
    else:
        jsonld_data = data
...
def serialize_sequence(
    ...
    if format == "dict":
        ...
        if include_context:
            context = _load_jsonld_context()
            jsonld_data = {"@context": context, "transforms": sequence_data}
        else:
        ...
        raise ValueError(
            f"Unsupported format: {format}. Use 'json', 'jsonld', or 'dict'."
        )
