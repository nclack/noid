...
def _validate_transform_structure(transform: Transform) -> None:
    ...
    from .models import (
        Identity, Translation, Scale, MapAxis, Homogeneous,
        DisplacementLookupTable, CoordinateLookupTable
    )
    
    # Check that transform is a known type
    if not isinstance(transform, (Identity, Translation, Scale, MapAxis, 
                                Homogeneous, DisplacementLookupTable, CoordinateLookupTable)):
    ...
    if isinstance(transform, Translation):
        if not hasattr(transform, 'translation') or not transform.translation:
            raise ValidationError("Translation transform must have non-empty translation vector")
        ...
        if not hasattr(transform, 'mapAxis') or not transform.mapAxis:
            raise ValidationError("MapAxis transform must have non-empty mapAxis vector")
        if not all(isinstance(x, int) for x in transform.mapAxis):
            raise ValidationError("MapAxis vector must contain only integers")
        if any(x < 0 for x in transform.mapAxis):
        ...
        if hasattr(transform, '_rows') and hasattr(transform, '_cols'):
            ...
            if len(transform.homogeneous) != expected_size:
                raise ValidationError(f"Homogeneous matrix size mismatch: expected {expected_size}, got {len(transform.homogeneous)}")
...
def _validate_transform_semantics(transform: Transform) -> None:
    ...
    from .models import (
        Translation, Scale, MapAxis, Homogeneous,
        DisplacementLookupTable, CoordinateLookupTable
    )
    ...
    if isinstance(transform, MapAxis):
        ...
        if len(set(transform.mapAxis)) != len(transform.mapAxis):
        ...
        max_index = max(transform.mapAxis)
        if max_index > 10:  # Arbitrary threshold
            import warnings
            warnings.warn(f"MapAxis contains very large index {max_index}, which may indicate an error")
        ...
        if hasattr(transform, '_rows') and hasattr(transform, '_cols'):
            if transform._rows != transform._cols:
                import warnings
                warnings.warn(f"Homogeneous matrix is not square ({transform._rows}x{transform._cols})")
    
    elif isinstance(transform, (DisplacementLookupTable, CoordinateLookupTable)):
        # Validate sampler configuration if present
        config = getattr(transform, 'displacements', None) or getattr(transform, 'lookup_table', None)
        if config:
...
def _validate_sampler_config(config) -> None:
    ...
    if not isinstance(config, SamplerConfig):
        raise ValidationError(f"Sampler configuration must be SamplerConfig, got {type(config)}")
...
def validate_sequence(transforms: List[Transform], strict: bool = True) -> bool:
    ...
    for i, transform in enumerate(transforms):
        try:
            ...
            if strict:
                raise ValidationError(f"Transform {i} failed validation: {e}")
            return False
    
    return True


def validate_dimension_consistency(transforms: List[Transform], strict: bool = True) -> bool:
    """
    ...
    Example:
        ...
        True
    """
    from .models import Translation, Scale, MapAxis
    ...
    for i, transform in enumerate(transforms):
        if isinstance(transform, (Translation, Scale)):
            dim = len(transform.translation if isinstance(transform, Translation) else transform.scale)
            dimensions.append((i, dim))
        elif isinstance(transform, MapAxis):
            input_dim = max(transform.mapAxis) + 1 if transform.mapAxis else 0
            output_dim = len(transform.mapAxis)
...
def check_transform_compatibility(transform1: Transform, transform2: Transform) -> bool:
    ...
    Example:
        ...
        True
    """
    from .models import Translation, Scale, MapAxis, Identity
    ...
    if isinstance(transform1, (Translation, Scale)) and isinstance(transform2, (Translation, Scale)):
        dim1 = len(transform1.translation if isinstance(transform1, Translation) else transform1.scale)
        dim2 = len(transform2.translation if isinstance(transform2, Translation) else transform2.scale)
        return dim1 == dim2
    ...
    if isinstance(transform1, MapAxis) and isinstance(transform2, (Translation, Scale)):
        output_dim1 = len(transform1.mapAxis)
        dim2 = len(transform2.translation if isinstance(transform2, Translation) else transform2.scale)
