...
class TestNamespaceContext:
    ...
    def test_set_namespace_basic(self):
        ...
        test_registry = TransformRegistry()
    ...
    def test_collision_detection(self):
        ...
        assert "https://collision.test/transform1" in test_registry.get_registered_iris()
        ...
        with pytest.raises(RuntimeError, match="already registered with a different factory"):
...
class TestFactoryRegistration:
    ...
    def test_name_override_registration(self):
        ...
        @self.registry.register("mapAxis")  # Override snake_case → camelCase
        def map_axis(data: List[int]) -> MockTransform:
            return MockTransform(data)
        
        # Should be registered with override name
        result = self.registry.create("https://test.com/schemas/mapAxis", [0, 1, 2])
...
class TestErrorHandling:
    ...
    def test_unknown_transform_error(self):
        ...
        with pytest.raises(UnknownTransformError) as exc_info:
        ...
        assert "Unknown transform IRI" in str(error)
    
    def test_unknown_transform_error(self):
        """Test clean error messages for unknown transforms."""
        with pytest.raises(UnknownTransformError) as exc_info:
            self.registry.create("https://test.com/schemas/unknown_transform", [1, 2, 3])
        
        error = exc_info.value
        error_msg = str(error)
        # Should provide clean error message
        assert "Unknown transform IRI: 'https://test.com/schemas/unknown_transform'" in error_msg
    ...
    def test_factory_validation_error(self):
        ...
        with pytest.raises(FactoryValidationError) as exc_info:
            self.registry.create("https://test.com/schemas/failing_transform", [1, 2, 3])
...
class TestRegistryIntegration:
    ...
    def test_registration_persistence(self):
        ...
        result = registry.create("https://global.test/persistent_transform", "test_data")
        assert isinstance(result, MockTransform)
        assert result.data == "test_data"
        
        # Should appear in registered IRIs
        assert "https://global.test/persistent_transform" in registry.get_registered_iris()
